...
class Checkers:
    ...
    def CompTurn(s):
        ...
        for move in s.moves:
            s.goodMoves.append([0,move])
        
        #To prevent leaving the back row (currently top priority)
        for move in s.goodMoves:
            if s.movesFromBack(move[1]):
               print 'bad'# s.badMoves.append(move[1])
            else:
                move[0] += 6
#         s.removeBadMoves()
        ...
        for move in s.goodMoves:
            if not s.PieceCanCapture(s.moveEndsAt(move[1])[0],s.moveEndsAt(move[1])[1]):
               print 'bad'# s.badMoves.append(move[1])
            else:
                move[0]+= 5
#         s.removeBadMoves()
        ...
        for move in s.goodMoves:
            if not (((s.moveEndsAt(move[1])[1]==7 and s.tiles[move[1][0]][move[1][1]].isWhite) or \
               (s.moveEndsAt(move[1])[1]==0 and s.tiles[move[1][0]][move[1][1]].isBlack)) and \
                    s.tiles[move[1][0]][move[1][1]].isPawn):
               print 'bad'# s.badMoves.append(move[1])
            else:
                move[0]+= 4
#         s.removeBadMoves()
        ...
        for move in s.goodMoves:
            if not ((s.tiles[move[1][2]][move[1][3]].isKing) and s.isMoveSafe(move[1])):
               print 'bad'# s.badMoves.append(move[1])
            else:
                move[0]+= 3
#         s.removeBadMoves()
        ...
        for move in s.goodMoves:
            if not ((s.tiles[move[1][0]][move[1][1]].isPawn) and (s.tiles[move[1][2]][move[1][3]].isKing) and not s.isMoveSafe(move[1])):
               print 'bad'# s.badMoves.append(move[1])
            else:
                move[0]+= 2
#         s.removeBadMoves()
        ...
        for move in s.goodMoves:
            if not ((s.tiles[move[1][0]][move[1][1]].isKing) and (s.tiles[move[1][2]][move[1][3]].isKing) and not s.isMoveSafe(move) and s.hasMorePieces()):
               print 'bad'# s.badMoves.append(move[1])
            else:
                move[0] += 1
            #s.removeBadMoves()
        ...
        for move in s.goodMoves:
            if not ((s.tiles[move[1][0]][move[1][1]].isPawn) and (s.tiles[move[1][2]][move[1][3]].isPawn) and not s.isMoveSafe(move) and s.hasMorePieces()):
               print 'bad'# s.badMoves.append(move[1])
            else:
                move[0] += 1
#         s.removeBadMoves()
...
#         s.removeBadMoves()
        ...
        if (len(s.goodMoves)):
            highestMove = [0, '']
            for move in s.goodMoves:
                if move[0] > highestMove[0]:
                    highestMove = move
            
            s.Action(highestMove[1][2], highestMove[1][3])
        #else:
#         m = randrange(0,len(s.moves))
#         if s.selectedTileAt == []:
#             s.Action(s.moves[m][0],s.moves[m][1])
#         s.Action(s.moves[m][2],s.moves[m][3])
#         tkMessageBox.showinfo("Error",s.moves[m])
